...
> **학습 목표:** 쉘 스크립트의 기초 구조(Shebang, 변수, 매개변수)와 제어문(조건문 `if`, 반복문 `for`)을 익히고, 디버깅 방지 규칙을 적용하여 자동화 스크립트 및 백업 스크립트를 작성합니다.
...
## 3. 조건문(if) 및 안전한 조건식 검사

서버 자동화 스크립트에서는 파일/디렉토리 존재 여부와 문자열/숫자를 비교할 때 디버깅 오류를 방지하는 규칙이 매우 중요합니다.
...
### 3.2 [실무 필수] 숫자 vs 문자열 비교 연산자 구분
* **숫자 비교 연산자:** `-eq` (같음), `-ne` (다름), `-gt` (크다), `-ge` (크거나 같다), `-lt` (작다), `-le` (작거나 같다)
* **문자열 비교 연산자:** `==` 또는 `=` (같음), `!=` (다름), `-z` (빈 문자열)

### 3.3 [실무 팁] 조건식 변수는 반드시 쌍따옴표(`"$VAR"`) 감싸기
변수 값이 비어 있거나 공백이 포함된 경우 `unary operator expected` 에러로 스크립트가 중단되는 것을 방지합니다.
...
## 6. 스크립트 디버깅 및 오류 추적 (`bash -x`)

스크립트가 생각대로 실행되지 않을 때 한 줄씩 구문을 추적하며 변수 할당과 실행 결과를 확인하는 디버깅 옵션입니다.

```bash
# 한 줄씩 실행 과정을 출력하며 디버깅 실행
bash -x /opt/scripts/auto_backup.sh
```
...
| **비교 연산자** | 숫자: `-eq`, `-gt` / 문자: `==`, `!=` | 비교 대상 타입에 맞춰 명확히 구분 |
| **안전한 조건식** | `[ -d "$DIR" ]` | 대괄호 양옆 공백 및 **변수 쌍따옴표** 필수 |
| **디버깅 실행** | `bash -x script.sh` | 구문 실행 과정 추적 모드 |
