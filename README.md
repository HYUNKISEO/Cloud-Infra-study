# 🚀 Cloud Engineer & Developer's Infrastructure Study Journey

> **"개발자 & 클라우드 엔지니어를 향한 여정"**  
> 본 저장소는 KDT-웹개발을 기반으로 리눅스 시스템 마스터를 거쳐 클라우드 인프라 아키텍처 전문가 및 개발자로 성장하기 위한 **치열한 학습 기록장**입니다. 🔥

---

## 🗺️ Master Roadmap

| Phase | 주제 및 역량 목표 | 기간 / 학습량 | 상태 | 상세 기록 |
| :---: | :--- | :---: | :---: | :---: |
| **01** | **Linux 기초 및 시스템 관리** | 15 Days | ✅ 완료 | [`📁 01-Linux-Basic`](./01-Linux-Basic) |
| **02** | **클라우드 네트워크 & 보안 (IP, DNS, VPC, Firewall)** | 10 Days | 🔥 진행 예정 | [`📁 02-Cloud-Network`](./02-Cloud-Network) |
| **03** | **쉘 스크립트(Bash) & 인프라 자동화 기초** | - | ⏳ 대기중 | - |
| **04** | **AWS Core Services (EC2, S3, IAM, ALB)** | - | ⏳ 대기중 | - |
| **05** | **컨테이너 기반 인프라 (Docker & Kubernetes)** | - | ⏳ 대기중 | - |
| **06** | **IaC & DevOps 자동화 (Terraform, CI/CD)** | - | ⏳ 대기중 | - |

---

## 🐧 Phase 1. Linux System Administrator (Completed ✅)

> 클라우드 서버의 뼈대가 되는 리눅스 핵심 명령어, 권한/소유권, 서비스 관리(`systemctl`), 네트워크/로그 트러블슈팅, 크론탭 스케줄링, 쉘 스크립트 기초까지 **15일간의 실무 학습을 성공적으로 완수**했습니다.

* 📖 **일별 학습 목차 & 노하우 정리:** [`01-Linux-Basic 폴더 바로가기`](./01-Linux-Basic)

---

## 🌐 Phase 2. Cloud Network & Security (Coming Soon 🔥)

> AWS, GCP 등 클라우드 인프라의 핵심이 되는 네트워크 구성 요소(IP, Subnetting, Routing, DNS, VPC, Security Group) 및 보안 메커니즘을 완벽히 이해합니다.

* 📖 **일별 학습 목차 & 노하우 정리:** [`02-Cloud-Network 폴더 바로가기`](./02-Cloud-Network)

---

## 🛠️ My Tech Stack & Tools

* **OS:** Ubuntu Linux / CentOS
* **Network & Security:** Wireshark, `tcpdump`, `traceroute`, `curl`, VPC Architecture
* **VCS & Tools:** Git & GitHub, VS Code, Nano
* **Mindset:** "손으로 치고, 부딪히고, 기록한다." 💡
