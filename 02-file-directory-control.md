...
> 💡 **[심화 참고] 작업 속도를 2배 올리는 단축어 & 이력 검색 (`alias` & `Ctrl + R`)** <br>
> 실무 엔지니어들은 자주 쓰는 길고 복잡한 명령어를 **`alias`**로 나만의 단축어로 등록하거나, 과거에 쳤던 긴 명령어를 **`Ctrl + R`** 검색으로 순식간에 불러와서 재사용합니다.
> * `alias ll='ls -alF'` ➡️ 터미널에 `ll`만 입력해도 상세 목록 조회 명령어 실행
> * alias는 여러개 만들어 둘 수 있으며, 계정귀속이다.
> * **`Ctrl + R`** 누른 후 키워드(예: `nginx`) 입력 ➡️ 과거에 실행했던 관련 명령어 자동 완성
